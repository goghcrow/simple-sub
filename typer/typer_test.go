...
import (
	...
	"github.com/goghcrow/simple-sub/front/parser"
	"github.com/goghcrow/simple-sub/front/trans"
...
func parsePgrm(s string) *terms.Program {
	ops := oper.BuildIn()
	lex := lexer.NewLexer(ops)
	desugar := trans.Desugar(parser.NewParser(ops).Parse(lex.Lex(s)))
