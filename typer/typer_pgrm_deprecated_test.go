...
import (
	...
	"github.com/goghcrow/simple-sub/deprecated/parser"
	parser2 "github.com/goghcrow/simple-sub/parser"
	"github.com/goghcrow/simple-sub/terms"
	"testing"
)

func parsePgrm_deprecated(s string) *terms.Program {
	ops := oper.BuildIn()
	lex := lexer.NewLexer(ops)
	desugar := parser2.Desugar(parser.NewParser(ops).Parse(lex.Lex(s)))
	return desugar.(*terms.Program)
}

func TestPgrm_deprecated(t *testing.T) {
	var typer = NewTyper()
...
`,
		...
		t.Run(tt.name, func(t *testing.T) {
			pgrm := parsePgrm_deprecated(tt.pgrm)
